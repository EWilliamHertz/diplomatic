...
export const Leads = () => {
  ...
  const { leads, importLeads, updateLeadStatus, bulkUpdateLeadStatus, deleteLead, bulkDeleteLeads } = useStore();
  const [isImportOpen, setIsImportOpen] = useState(false);
  const [view, setView] = useState<'table' | 'kanban'>('table');
  const [sortCol, setSortCol] = useState<string | null>(null);
  const [sortAsc, setSortAsc] = useState(true);

  // Extract all unique columns from the leads
  const columns = Array.from(new Set(leads.flatMap(l => Object.keys(l).filter(k => k !== 'id' && k !== 'status'))));
  ...
  const selectNext = (count: number) => {
    const unselected = sortedLeads.filter(l => !selectedIds.includes(l.id)).slice(0, count);
  ...
  return (
    <div className="flex flex-col gap-6" style={{ height: 'calc(100vh - 120px)' }}>
      <div className="flex justify-between items-center" style={{ flexShrink: 0 }}>
        <h2 style={{ fontSize: '24px' }}>{t('nav.leads') ?? 'Leads'}</h2>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }} className="flex flex-col gap-4">
        {selectedIds.length > 0 && (
          <div className="panel py-3 px-4 flex items-center justify-between border-mint bg-mint/10" style={{ borderStyle: 'solid', borderWidth: '1px' }}>
             <div className="font-bold text-mint">{selectedIds.length} leads selected</div>
        ...
        {leads.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-secondary panel h-full" style={{ borderStyle: 'dashed' }}>
            <p>No leads found. Import an Excel file to get started.</p>
          </div>
        ) : view === 'table' ? (
          <div className="panel" style={{ overflowX: 'auto' }}>
            <div className="flex gap-2 mb-4">
              <button onClick={toggleSelectAll} className="btn btn-secondary text-xs px-2 py-1">
                {selectedIds.length === sortedLeads.length ? 'Deselect All' : 'Select All'}
              </button>
            ...
            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
                ...
                <tr style={{ borderBottom: '1px solid var(--panel-border)', color: 'var(--text-secondary)' }}>
                  <th className="py-3 font-medium px-4 w-10">
                    <button onClick={toggleSelectAll} className="text-secondary hover:text-white">
                      {selectedIds.length > 0 && selectedIds.length === sortedLeads.length ? <CheckSquare size={16} className="text-mint" /> : <Square size={16} />}
                    </button>
                  </th>
              ...
              <tbody>
                {sortedLeads.map(lead => (
                  <tr key={lead.id} className="group hover:bg-white/5 transition cursor-pointer" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }} onClick={() => toggleSelect(lead.id)}>
                    <td className="py-4 px-4" onClick={e => e.stopPropagation()}>
                      <button onClick={() => toggleSelect(lead.id)} className="text-secondary hover:text-white mt-1">
                        {selectedIds.includes(lead.id) ? <CheckSquare size={16} className="text-mint" /> : <Square size={16} />}
                      </button>
                    </td>
                    <td className="py-4 px-4" onClick={e => e.stopPropagation()}>
                      <select 
                    ...
                    {columns.map(col => (
                      <td key={col} className="py-4 px-4 text-sm text-gray-300">{lead[col] || '-'}</td>
                    ))}
                    <td className="py-4 px-4" onClick={e => e.stopPropagation()}>
                      <button 
          ...
          <div className="grid grid-cols-4 gap-4 h-full" style={{ minWidth: '800px' }}>
            {kanbanColumns.map(status => (
              <div key={status} className="panel flex flex-col gap-4 h-full bg-black/40 border border-white/5">
                <h3 className="font-bold text-secondary uppercase tracking-widest text-xs border-b border-white/10 pb-2">{status} ({leads.filter(l => l.status === status).length})</h3>
                <div className="flex flex-col gap-3 overflow-y-auto">
                  {leads.filter(l => l.status === status).map(lead => (
                    <div key={lead.id} className="p-3 rounded border border-white/10 bg-black/60 shadow-lg cursor-pointer hover:border-mint/50 transition relative group">
                      <button 
                        onClick={() => deleteLead(lead.id)}
                      ...
                      {columns.slice(0, 3).map(col => (
                        <div key={col} className="text-sm pr-6"><span className="text-secondary text-xs">{col}:</span> {lead[col] || '-'}</div>
                      ))}
                      <select 
                        value={lead.status} 
                        onChange={(e) => updateLeadStatus(lead.id, e.target.value)}
      ...
      <ImportModal isOpen={isImportOpen} onClose={() => setIsImportOpen(false)} target="Leads" onImport={importLeads} />
    </div>
  );
};
